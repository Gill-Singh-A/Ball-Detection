...
if __name__ == "__main__": # If this program is the main namespace then only run it
    ...
    while True: # Running the Program till ’q’ is pressed
        ...
        gray_scale_frame_blurred = cv2.blur(gray_scale_frame, (3, 3)) # blurring the image to get more accuracy and decrease load on cpu
        gray_scale_frame_blurred = cv2.blur(gray_scale_frame_blurred, (3, 3)) # blurring even more for thesame
        ...
        if balls is not None: # checks if there is a ball or not
            ...
            for ball in balls[0]: # iterate through the detected balls
                ...
                cv2.imshow("Ball Detection", original_frame) # Showing the image with marked balls
